...
public class newMediaDatabase extends SQLiteOpenHelper {
    private static final String dbname = "newMedia.db";

    public newMediaDatabase(@Nullable Context context) {
        super(context, dbname, null, 1);
    }

    @Override
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        String q = "create table newMedia ( size text ,format text ," +
    ...
    public Cursor getFileNameData() {
        ...
        Cursor cursor = dbMedia.rawQuery("select * from newMedia ", null);
        return cursor;
    }
    ...
    public boolean checkDbIsEmpty() {
        ...
        Cursor mCursor = dbMedia.rawQuery("SELECT * FROM " + "newMedia", null);
        Boolean rowExists;
    ...
    public void clearDatabase() {
        SQLiteDatabase dbMedia = this.getWritableDatabase();
        String clearDBQuery = "DELETE FROM " + "newMedia";
    ...
    public boolean insert_data(
        ...
        long r = dbMedia.insert("newMedia", null, c);
        return r != -1;
    ...
    public boolean updateData(String fileName) {
        ...
        Cursor cursor = dbMedia.rawQuery("select * from newMedia where fileName=?", new String[]{fileName});
        if (cursor.getCount() > 0) {
            long r = dbMedia.update("newMedia", c, "fileName=?", new String[]{fileName});
            return r != -1;
        } else return false;
    }
    ...
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int i, int i1) {
        sqLiteDatabase.execSQL("drop table if exists newMedia");
...
*/
    ...
    public ArrayList<String> getDownloadedFileList(String type) {
        ...
        Cursor cursor = database.rawQuery("select * from newMedia where isDownloaded=?", new String[]{"1"});
        try {
            if (cursor.moveToFirst()) {
                do {
    ...
    public ArrayList<String> getPendingFileNames() {
        ...
        SQLiteDatabase database = this.getWritableDatabase();
        Cursor cursor = database.rawQuery("select * from newMedia where isDownloaded=?", new String[]{"0"});
        try {
            if (cursor.moveToFirst()) {
                do {
    ...
    public void deleteARow(String fileName) {
        SQLiteDatabase database = this.getWritableDatabase();
        long result = database.delete("newMedia", "fileName=?", new String[]{fileName});
        if (result == -1) {
